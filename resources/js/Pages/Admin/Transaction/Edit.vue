  ...
  <AppAdmin>
    ...
    <form class="my-8" @submit.prevent="submit">
      ...
      <div class="mb-4 flex flex-col gap-2">
        ...
        <input type="date" v-model="form.order_date" class="border-slate-200 w-full rounded-lg"
      ...
      <!-- Button -->
      <button type="submit" class="bg-blue-500 py-2 px-4 rounded-lg text-white">Update Transaction</button>
    </form>
  </AppAdmin>
</template>
...
const props = defineProps({ users: Object, transaction: Object, venues: Object })

const form = useForm({
  user_id: props.transaction.user_id ?? '',
  venue_id: props.transaction.venue_id ?? '',
  order_date: props.transaction.order_date ?? null,
  start_time: props.transaction.start_time ?? null,
  total_guest: props.transaction.total_guest ?? 0,
  payment_status: props.transaction.payment_status ?? '',
})

function submit() {
  form.put(`/admin/transaction/${props.transaction.id_transaction}/update`, {
    onSuccess: () => {
      alert('Success Update Transaction')
    }
  })
}
